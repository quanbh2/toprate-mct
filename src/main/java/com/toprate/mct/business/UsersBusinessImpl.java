...
import com.toprate.base.utils.DataUtils;
import com.toprate.base.utils.DateUtil;
import com.toprate.base.utils.PasswordUtil;
import com.toprate.base.utils.QlanResource;
import com.toprate.mct.bo.Users;
import com.toprate.mct.dao.ObjectsDAO;
import com.toprate.mct.dao.RoleUserDAO;
import com.toprate.mct.dao.RolesDAO;
import com.toprate.mct.dao.UsersDAO;
...
//		}
	...
	private String createJWT(String id, String issuer, String subject, long ttlMillis) {
	 ...
	 public ResultDTO changePassword(LoginDTO loginDTO)throws Exception{
		 ...
		 if(StringUtils.isNotEmpty(loginDTO.getUsername()) 
			 ...
			 if(!hashPassOld.equals(usersDTO.getPassword())){
				 resultDTO.setError(true);
				 resultDTO.setMessage(QlanResource.get("invalid.password"));
				 return resultDTO;
			 }
	 ...
	 public DataListDTO doSearchCustomer(UsersDTO obj) {
			List<UsersDTO> ls = usersDAO.doSearchCustomer(obj);
			DataListDTO data = new DataListDTO();
			data.setData(ls);
			data.setTotal(obj.getTotalRecord());
			data.setSize(obj.getTotalRecord());
...
//			actionAuditBusinessImpl.trackingAdjustment(Constants.ACTION_AUDIT_TYPE.INSERT,TABLE_NAME, usersDTO.getUserId(), newObj, usersDTO, QlanResource.get(DISPLAY_NAME_KEY), request);
		...
		public UsersDTO checkCustomerNameExit(String userName){
			return usersDAO.checkCustomerNameExit(userName);
		}
		
		public List<RoleUserDTO> getListRoleByUserId(Long userId){
//			return null;
			return rolesDAO.getListRoleByUserId(userId);
		}
		@Transactional
		public void insertUserRoleData(UsersDTO userDto) throws Exception {
			roleUserDAO.deleteByUserId(userDto.getUserId());
			if(userDto.getListRole().size()>0){
			for(RoleUserDTO roleUser:userDto.getListRole()){
				roleUserDAO.insert(roleUser,userDto);
			}
			}
		}
		...
		@Transactional
		public ResultDTO loginCustomer(String userName, String password, HttpServletRequest request) throws Exception {
			HttpSession httpSession=request.getSession();
			ResultDTO resultDTO = new ResultDTO();
			// check null username or password
			if (!StringUtils.isNotEmpty(userName) || !StringUtils.isNotEmpty(password)) {
			...
			String hasdPass = PasswordUtil.hashPassword(password, null);
			UsersDTO usersDTO = usersDAO.getCustomerInfo(userName);
			// check username invalid db
			if (usersDTO == null) {
				resultDTO.setMessage(QlanResource.get(INVALID_ACC_KEY));
				resultDTO.setError(true);
...
//			}
			...
			String jwt=createJWT(httpSession.getId(), null, userName, 1000000000000000l);
			httpSession.setAttribute("USER_TOKEN_CUSTOMER", jwt);
			httpSession.setAttribute("customerInfo", usersDTO);
//			List<ObjectsDTO> listObjParent=objectsDAO.getListObjects(usersDTO.getUserId(), null);
...
//			usersDTO.setListObject(listObjParent);
		...
		public UsersDTO getCustomerInfo(String userName) {
			return usersDAO.getCustomerInfo(userName);
		}
}
