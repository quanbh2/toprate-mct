...
public class UsersDAO extends BaseFWDAOImpl<Users, Long> {
	...
	public UsersDTO getUsersInfo(String userName){
		...
		query.addScalar("userType", new LongType());
	...
	public void resetPass(Long userId,String pass){
		StringBuilder sql = new StringBuilder("UPDATE users SET PASSWORD=:pass,LAST_CHANGE_PASSWORD=null,LOGIN_FAILURE_COUNT=0,STATUS=1 WHERE USER_ID=:userId ");
	...
	public UsersDTO checkUserNameExit(String userName){
		StringBuilder sql = new StringBuilder("SELECT "
				...
				+"users.LAST_LOCK_DATE AS lastLockDate "
				+"FROM users "
				+"WHERE upper(users.USER_NAME) = upper(:userName)  AND users.`STATUS`=1 AND users.USER_TYPE=1" );
		SQLQuery query= getSession().createSQLQuery(sql.toString());
	...
	@SuppressWarnings("unchecked")
	public List<UsersDTO> doSearchCustomer(UsersDTO obj){
		StringBuilder sql = new StringBuilder("SELECT "
				+ "users.USER_ID AS userId,"
				+ "users.USER_NAME AS userName,"
				+"users.FULL_NAME AS fullName, "
	...
	public UsersDTO getCustomerInfo(String userName){
		StringBuilder sql = new StringBuilder("SELECT "
				...
				+"users.LAST_LOCK_DATE AS lastLockDate "
				+"FROM users "				
				+"WHERE upper(users.USER_NAME) = upper(:userName)  AND users.USER_TYPE <> 1  " );
		SQLQuery query= getSession().createSQLQuery(sql.toString());
